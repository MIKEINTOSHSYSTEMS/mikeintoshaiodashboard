...
function calendar_init()
{
	...
	$calendarDatasourceTableNames["calendarFields"]["calendarnotiValue"] = "notiValue";
	$calendarDatasourceTableNames["calendarFields"]["calendarnotiName"] = "notiName";
	$calendarDatasourceTableNames["calendarFields"]["calendarnotiTime"] = "notiTime";
	$calendarDatasourceTableNames["calendarFields"]["calendarnotiUse"] = "notiUse";
}
...
function checkCalendarUserSettings() 
{
	...
	if ( isCalendarSettingsPropEmpty( "calendarKeys", "calendarid" ) ) 
		$message .= GetCustomLabel("keyfield_not_found").'"$calendarSettings[\'calendarKeys\'][\'calendarid\']".<br>';
	...
	if ( isCalendarSettingsPropEmpty( "calendarFields", "calendarDateField" ) && isCalendarSettingsPropEmpty( "calendarFields", "calendarDateTime" ) )
		$message .= GetCustomLabel("keyfield_not_found").'"$calendarSettings[\'calendarFields\'][\'calendarDateField\']".<br>';
	
	if ( isCalendarSettingsPropEmpty( "calendarFields", "calendarSubject" ) ) 
		$message .= GetCustomLabel("keyfield_not_found").'"$calendarSettings[\'calendarFields\'][\'calendarSubject\']".<br>';
	
//	if ( isCalendarSettingsPropEmpty( "calendarFields", "calendarEndDate" ) ) 
//		$message .= GetCustomLabel("keyfield_not_found").'"$calendarSettings[\'calendarFields\'][\'calendarEndDate\']".<br>';
	...
	{
		$message .= GetCustomLabel("keyfield_not_found").'"$calendarSettings[\'calendarFields\'][\'calendarTimeField\']".<br>';
	}
	...
	{
		$message .= GetCustomLabel("keyfield_not_found").'"$calendarSettings[\'calendarFields\'][\'calendarPeriodDays\']" or "$calendarSettings[\'calendarFields\'][\'calendarPeriod\']"<br>';
	}
...
/*	
	...
	{
		$message .= GetCustomLabel("keyfield_not_found").'"$calendarSettings[\'calendarFields\'][\'calendarRecurrence\']" , "$calendarSettings[\'calendarFields\'][\'calendarPeriod\']" and "$calendarSettings[\'calendarFields\'][\'calendarPeriodDays\']" <br>';
	}
*/
	...
	if ( !isCalendarSettingsPropEmpty( "calendarFields", "calendarPeriod" ) && ( isCalendarSettingsPropEmpty( "calendarFields", "calendarRecurrence" ) || 
		isCalendarSettingsPropEmpty( "calendarFields", "calendarPeriodDays" ) ) )
	{
		$message .= GetCustomLabel("keyfield_not_found").'"$calendarSettings[\'calendarFields\'][\'calendarPeriodDays\']" or "$calendarSettings[\'calendarFields\'][\'calendarRecurrence\']"<br>';
	}
	...
	{
		$message .= GetCustomLabel("keyfield_not_found").'"$calendarSettings[\'calendarFields\'][\'calendarPeriod\']" or "$calendarSettings[\'calendarFields\'][\'calendarRecurrence\']"<br>';
	}
...
function calendar_getListOfEventsInRange($rangeFrom, $rangeTo, $searchWhereClause, $connection)
{
	...
	if( $strPerm==1 && !$perm["M"])
		$whereOwnerID = $connection->addFieldWrappers($pSet->getTableOwnerID())."='".((string)$_SESSION["_calcalendar_OwnerID"])."'";
	...
	if( isset($event) && count($event) )
	{
		...
		if( count($event) > $_SESSION["calendar_CountEvents"] ) 
		{
			$eventsHTML .= "<a href='#' id=moreButton".$fireDbDate[2]." yr=".$fireDbDate[0]. " mon=" . $fireDbDate[1] 
				. " days=" .$fireDbDate[2].">".GetCustomlabel("more")."</a> (".GetCustomlabel("total").": ".count($event).")";
		}
...
function calendar_tempValidateEvent(&$values, &$ret, &$message)
{
	...
	{
		  $message="<span  class=message calendar_error>".GetCustomLabel("invalid_event_date")."</span>";
			 $ret=false;
	}
	else
	{
		...
		{
			$message="<span class=message calendar_error>".GetCustomLabel("invalid_event_date_or_time")."</span>";
			$ret=false;
		}
		else if (calendarFieldIsset(calendarGetSetting("calendarEndTime")) && comparedates($dbDateTimeEnd, $dbDateTime)<0) {
			$message="<span class=message calendar_error>".GetCustomLabel("invalid_event_time")."</span>";
			$ret=false;
		}
		...
		{
			$message="<span class=message calendar_error>".GetCustomlabel("select_the_day_of_the_week")."</span>";
			$ret=false;
		}
		else 
		{
...
function setEndValue($eventEnd, $data){
	...
	if($recurType == "Never"){
		$add_days = 0;
		for($i=0;$i<10;$i++){
			if(isleapyear( date("Y", strtotime($eventStart) ) + $i))
				$add_days+=366;
			else
				$add_days+=365;
		}
