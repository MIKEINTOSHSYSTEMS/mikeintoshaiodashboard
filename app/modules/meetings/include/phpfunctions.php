...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="mtmeetings" && $field=="createlink")
	{
		if(Security::isLoggedIn()){
	$user = Security::getUserData(Security::getUserName());
	if($user["id"] == $data["ownerid"] || Security::getUserName()=="admin")
		$value = "<a target=_blank href='".$value."' class='btn btn-default'>Start the Meeting</a>";
	else
		$value = "";
}
else
	$value = "<a target=_blank href='".$value."' class='btn btn-default'>Start the Meeting</a>";;
		return $value;
				...
				if($table=="mtmeetings" && $field=="joinlink")
	{
		if(Security::isLoggedIn()){
	...
	if(in_array($info[$id],$arrid) && $data["ownerid"]!=$info[$id] || Security::getUserName()=="admin" || !Security::isLoggedIn())
		$value = "<a target=_blank href='".$value."' class='btn btn-default'>Join the Meeting</a>";
	else
		$value = "";
}
else
	$value = "<a target=_blank href='".$value."' class='btn btn-default'>Join the Meeting</a>";;
		return $value;
				...
				if($table=="mtmeetings" && $field=="clink")
	{
		if(strlen($value)>80)
	$value = substr($value,0,78)."...";;
		return $value;
	}
				if($table=="mtmeetings" && $field=="hiddenclink")
	{
		$value="<input type=text value='".$value."' id='input_clink' size=76>";;
		return $value;
	}
				if($table=="mtmeetings" && $field=="ownerid")
	{
		$rs = DB::Query("select * from mtsettings");
$record = $rs->fetchAssoc();
$id = $record["idfield"];
if(Security::isLoggedIn()){
	$user = Security::getUserData(Security::getUserName());
	if($user[$id] == $value || Security::getUserName()=="admin" || !Security::isLoggedIn())
		$value = "<a href='javascript:void(0);' class='btn btn-default sendnoti' recid='".$data[$id]."'>Send notification</a>";
	else
		$value = "";
}
else
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="mtsettings" && $field=="provider")
	{
		return "Zoom";
	}
				if($table=="mtmeetings" && $field=="notiday")
	{
		return 0;
	}
