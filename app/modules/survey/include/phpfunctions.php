...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="response" && $field=="status")
	{
		if ($_SESSION['UserID']!='Guest'){
	if ($data["active"]==1)
	{
	$value= "&nbsp;&nbsp;&nbsp;<font color=red><b>Active</b></font>&nbsp;&nbsp;<a href='#' onclick='showActivWin(\"deactivate\",". $data["id"].",\"".getArchiveSurveyTable($data["id"])."\"); return false;'>Deactivate</a>";
	$value.="&nbsp;&nbsp;&nbsp;<a target=_blank href='response_add.php?sid=" . $data["id"] . "'>Take&nbsp;survey</a>";
	$rs = CustomQuery("select count(*) as c from ".getSurveyTable($data["id"]));
	$d=db_fetch_array($rs);
	if ($d)
		if ($d["c"]>0)
			$value .= "&nbsp;&nbsp;&nbsp;<a href='sv_report.php?svid=".$data["id"]."'>View responses</a>";
	}
	else
	{
	$value= "&nbsp;&nbsp;&nbsp;<b>Inactive</b>&nbsp;&nbsp;<a href='#' onclick='showActivWin(\"activate\",". $data["id"].",\"".getArchiveSurveyTable($data["id"])."\"); return false;'>Activate</a>";
	}
}
else {
	if ($data['active']==1){
		...
		return $value;
				...
				if($table=="sv_surveys" && $field=="status")
	{
		if($data['id']){
	$params = "";
	...
	if(!$uid)
		$uid = $udata["cn"];
	if ($data["active"]==1){
		$rss = CustomQuery("select * from ".AddTableWrappers("sv_survey_".$data["id"])." where ".AddFieldWrappers("userID")."='".$uid."' order by id desc");
		if($ddata = db_fetch_array($rss)){
			if($ddata["submit"] == 1){
				$params = "&page=1&hash=".$ddata["hash"];
			}
			else{
				$params = "&page=".$ddata["lastpage"]."&hash=".$ddata["hash"];
			}
		}
	}
	if ($_SESSION['UserID']!='Guest'){
		...
		{
		$value= "&nbsp;&nbsp;&nbsp;<font color=red><b>Active</b></font>&nbsp;&nbsp;<a href='#' onclick='showActivWin(\"deactivate\",". $data["id"].",\"".getArchiveSurveyTable($data["id"])."\"); return false;'>Deactivate</a>";
		$value.="&nbsp;&nbsp;&nbsp;<a target=_blank href='response_add.php?sid=" . $data["id"].$params."'>Take&nbsp;survey</a>";
		$rs = CustomQuery("select count(*) as ".AddFieldWrappers("c")." from ".AddTableWrappers(getSurveyTable($data["id"])));
		$d=db_fetch_array($rs);
		if ($d)
			if ($d["c"]>0){
				$value .= "&nbsp;&nbsp;&nbsp;<a href='sv_report_list.php?svid=".$data["id"]."'>View responses</a>";
				$value .= "&nbsp;&nbsp;&nbsp;<a href='response_edit.php?sid=".$data["id"]."'>View stats</a>";
			}
		}
		else
		{
			$value = "&nbsp;&nbsp;&nbsp;<b>Inactive</b>&nbsp;&nbsp;";
			$rsq = CustomQuery("select count(*) as ".AddFieldWrappers("cq")." from ".AddTableWrappers("sv_questions")." where ".AddFieldWrappers("sid")."=".$data["id"]);
			$dq=db_fetch_array($rsq);
			if ($dq)
				if ($dq["cq"]>0)
					$value .= "<a href='#' onclick='showActivWin(\"activate\",". $data["id"].",\"".getArchiveSurveyTable($data["id"])."\"); return false;'>Activate</a>";
		}
	}
	else {
		if ($data['active']==1){
		...
		return $value;
				...
				if($table=="sv_report" && $field=="sv_view")
	{
		$value = "<a href='response_view.php?sid=".$_SESSION["svid"]."&resp=".$value."'>View</a>";;
		return $value;
	}
				if($table=="sv_report" && $field=="sv_export")
	{
		$value = "<a href='#' onclick='sv_exportLink(".$data["id"].",".$_SESSION["svid"].");return false;'>Export</a>";;
		return $value;
	}
				if($table=="sv_report" && $field=="procent")
	{
		$str = "select format from sv_surveys where id=".$_SESSION["svid"];
$ers = DB::Query($str);
$edata = $ers->fetchAssoc();
$format = $edata["format"];
$total = 0;
$num_group = 0;
$num_question = 0;
$group_name="";
$str = "select q.*,gr.name,gr.description from sv_questions as q inner join sv_groups as gr on gr.id=q.group_id where q.sid=".$_SESSION["svid"]." order by gr.sortorder,q.sortorder";
$ers = DB::Query($str);
while($edata = $ers->fetchAssoc())
{	
	if ($group_name!=$edata["name"])
	{
		$group_name=$edata["name"];
		$num_group++;
	}
	$num_question++;
}
...
$proc = number_format($data["answer"]*100/$total,2,".",",");
$value = "<div style='text-align:center'>".(string)($proc)." %</div>
<div class='ui-progressbar ui-widget ui-widget-content ui-corner-all' style='height:10px;text-align:center'>
   <div style='width: ".$proc."%;height:10px; background: none repeat scroll 0 0 green;' class='ui-progressbar-value ui-widget-header ui-corner-left'></div>
</div>";;
		return $value;
	}
	return $value;
...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table)
		$table = $strTableName;
		if($table=="sv_questions" && $field=="group_id")
	{
		$strWhere = " " .AddFieldWrappers("sid")."=".$_SESSION["sv_questions_masterkey1"];
		return $strWhere;
	}
		if($table=="sv_scenarios" && $field=="quest")
	{
		$strWhere = " ".AddTableWrappers("sv_questions").".".AddFieldWrappers("sid")."=".$_SESSION["sid"]." and ".AddTableWrappers("sv_questions").".".AddFieldWrappers("id")." in (".$_SESSION["curr_qid"].") and ".AddTableWrappers("sv_questions").".".AddFieldWrappers("type")." in ('T','E','L','N','D','R','X')";
		return $strWhere;
	}
	return "";
}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="response" && $field=="created")
	{
		return now();
	}
				if($table=="response" && $field=="format")
	{
		return "G";
	}
				if($table=="sv_surveys" && $field=="format")
	{
		return "G";
	}
				if($table=="sv_answer_options" && $field=="sortorder")
	{
		return $_SESSION["sv_answer_options_default_sortorder"]++;;
	}
				if($table=="sv_question_details" && $field=="sortorder")
	{
		return $_SESSION["sv_question_details_default_sortorder"]++;
	}
	return "";
}
