    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            destroy: function (e, data) {
				...
				if (!$('.template-download.in').length){
        ...
        _startHandler: function (e) {
            ...
            if (data && data.submit && !data.jqXHR && data.submit()) {
				...
				setTimeout(function(){
				if (!$('.template-upload.in').length){
					$(".btn.info.cancel").css('display','none');
				}
				},100);
            }
        },
        ...
        _transitionCallback: function (node, callback) {
            ...
            if (this._transition && node.hasClass('fade')) {
				...
				if ( ( /Trident\/7\./.test( navigator.userAgent ) ) && /^(10|11)./.test( $.browser.version ) ) {
        ...
        _initButtonBarEventHandlers: function () {
            ...
            fileUploadButtonBar.find('.delete')
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
                    $("input[class=delete]").each(function(){
						if ($(this).prop('checked')){
							$(this).parent().siblings('button').click();
						}
					});
                });
