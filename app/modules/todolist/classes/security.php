	...
	*  For AD it is <provider code><username>
	...
	public static function  fillTablesOwnerId( $data, $provider ) {
		...
		if( $securityType == stAD && !$provider["useDbGroups"] ) {
			...
			storageSet( "_calcalendar_OwnerID", $userId );
						storageSet( "_todogantt_OwnerID", $userId );
						storageSet( "_Settings_OwnerID", $userId );
						storageSet( "_todocomments_OwnerID", $userId );
		} else {
						storageSet( "OwnerID", $data["username"] );
			storageSet( "_calcalendar_OwnerID", $data["username"] );
						storageSet( "_todogantt_OwnerID", $data["username"] );
						storageSet( "_Settings_OwnerID", $data["id"] );
						storageSet( "_todocomments_OwnerID", $data["id"] );
		}
	}
