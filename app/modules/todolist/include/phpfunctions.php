...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="todoboards" && $field=="id")
	{
		global $cUserNameField;
$user = Security::currentUserData();
if($user[$cUserNameField] == $data["userid"])
	$value="<button id='delete_board_".$value."' boardid=".$value." class='btn btn-default btn-xs' style='display:none'>X</button>";
else
	$value="<span id='delete_board_".$value."' boardid=".$value." style='display:none'></span>";;
		return $value;
	}
				if($table=="todocategories" && $field=="color")
	{
		$value = "<span>".$value."</span>&nbsp;&nbsp;<span style='background-color:".$value."'>&nbsp;&nbsp;&nbsp;&nbsp;</span>";;
		return $value;
	}
				if($table=="todogantt" && $field=="categoryid")
	{
...
{
	$value = "";
				if($table=="todousers" && $field=="avatar")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
				...
				if($table=="calcalendar" && $field=="PeriodDays")
	{
		return setDefaultPeriodDay();;
				...
				if($table=="calcalendar" && $field=="OwnerID")
	{
