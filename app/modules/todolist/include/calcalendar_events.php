		...
		if($record = $rs->fetchAssoc()){
			...
			if($editable){
					...
					if(postvalue("mode")=="resize"){
						...
						if($calendarSettings["calendarFields"]["calendarEndDate"] && $calendarSettings["calendarFields"]["calendarEndTime"])
							$calendarEndDate = ", ".addFieldWrappers($calendarSettings["calendarFields"]["calendarEndDate"])."='".date("Y-m-d",strtotime($record[$calendarSettings["calendarFields"]["calendarEndDate"]])-$diff)."'";
						DB::Exec("update ".addTableWrappers($calendarSettings["calendarTable"])." set ".$calendarDayEvent.addFieldWrappers(calendarGetSetting(calendarGetDateOrTimeKey("date")))."='".date("Y-m-d",strtotime($record[calendarGetSetting(calendarGetDateOrTimeKey("date"))])-$diff)."'".$calendarEndDate.$calendarTimeField.$calendarEndTime." where ".addFieldWrappers($calendarSettings["calendarKeys"]["calendarid"])."=".postvalue("id"));
					}
					else{
						if($calendarSettings["calendarFields"]["calendarDayEvent"])
...
if(postvalue("a")=="monthly"){
	...
	$pageObject->pSet->setFullFieldName("Subject", "cardname" );
	...
	foreach($events as $ekey=>$values){
		foreach($values as $k=>$val){
			...
			if(!$val[$calendarSettings["calendarFields"]["calendarDayEvent"]] && $val[$calendarSettings["calendarFields"]["calendarTimeField"]] && $gStartTime && strtotime($val[$calendarSettings["calendarFields"]["calendarTimeField"]])<strtotime($gStartTime))
				continue;
			if(!$val[$calendarSettings["calendarFields"]["calendarDayEvent"]] && $val[$calendarSettings["calendarFields"]["calendarEndTime"]] && $gEndTime && strtotime($val[$calendarSettings["calendarFields"]["calendarEndTime"]])>strtotime($gEndTime))
				continue;
			...
			if($val[$calendarSettings["calendarFields"]["calendarSubject"]]){
				$rsl = DB::Select("todolists",array("id"=>$val[$calendarSettings["calendarFields"]["calendarRecurType"]]));
				$datal = $rsl->fetchAssoc();
				$rsb = DB::Select("todoboards",array("id"=>$datal["boardid"]));
				$datab = $rsb->fetchAssoc();
				$elem["title"] = $val[$calendarSettings["calendarFields"]["calendarSubject"]]." / ".$datal["listname"]." / ".$datab["boardname"];
			}
			else
				$elem["title"] = "<Empty>";
			if($val["color"])
...
$pageObject->AddJSFile("include/runnerJS/todo_calendar_lib.js");
$pageObject->AddJSFile("include/runnerJS/todo_calendar_buttons.js", "include/runnerJS/pages/AddPage.js");
$pageObject->AddJSFile("include/runnerJS/todo_calendar_add_edit.js", "include/runnerJS/pages/AddPage.js");
...
if(@$_SESSION["language"]){
	$rs = DB::Select("callocale",array("runnerlocale"=>$_SESSION["language"]));
	$data = $rs->fetchAssoc();
	$lang = $data["fullcalendarlocale"];
}
$pageObject->setProxyValue("lang",$lang);

$rs = DB::Query("select * from ".addFieldWrappers("calglobals")." order by ".addFieldWrappers("id"));
$glob = $rs->fetchAssoc();
$gStartTime = "00:00:00";
if($glob["StartTime"])
	$gStartTime = $glob["StartTime"];
...
function BeforeProcessEdit($pageObject)
{
		...
		$pageObject->AddJSFile("include/runnerJS/todo_calendar_add_edit.js", "include/runnerJS/pages/EditPage.js");
/*if($_REQUEST["delete1"])
{
...
function BeforeProcessView($pageObject)
{
		...
		$pageObject->AddJSFile("include/runnerJS/todo_calendar_add_edit.js", "include/runnerJS/pages/ViewPage.js");
...
function BeforeShowView(&$xt, &$templatefile, &$values, $pageObject)
{
		...
		//$pageObject->hideItem("description");
$pageObject->hideItem("timefield");
$pageObject->hideItem("enddate");
$pageObject->hideItem("endtime");
$pageObject->hideItem("dayevent");
$pageObject->hideItem("recurrence");
$pageObject->setProxyValue("color",$values["color"]);
// Place event code here.
// Use "Add Action" button to add code snippets.
;
