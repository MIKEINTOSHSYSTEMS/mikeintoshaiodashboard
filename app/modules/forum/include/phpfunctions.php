...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="forumreplies" && $field=="userid")
	{
		
DB::SetConnection(DB::ConnectionByTable(Security::getLoginTable()));
$data_rs = DB::Select(Security::getLoginTable(),array($_SESSION["loginKeyField"] =>$data["userid"] ));
$mydata=$data_rs->fetchAssoc();
DB::SetConnection(DB::DefaultConnection());
global $cDisplayNameField,$cUserpicField,$cUserNameField;
if($mydata)
	$value = $mydata[$cDisplayNameField];
$profileData = getUserData($data["userid"]);
if( is_null( $profileData["image"] )  ) {
	$value = "<div class='user-circle'><span class='user-initials'>".substr(strtoupper($value),0,1)."</span></div>";
} else {
	$value = "<img height='40px' class='avatar' src='avatars/".$profileData["image"]."'>";
}
if($_SESSION["user_id"])
	$value = "<a href='#' data-username='".$mydata[$cUserNameField]."' class='userprofile' displayname='".$mydata[$cDisplayNameField]."' userid=".$mydata[$_SESSION["loginKeyField"]].">".$value."</a>";;
		return $value;
	}
				if($table=="forumreplies" && $field=="userid2")
	{
		
DB::SetConnection(DB::ConnectionByTable(Security::getLoginTable()));
$rs = DB::Select(Security::getLoginTable(),array($_SESSION["loginKeyField"] => $value)); 
global $cDisplayNameField,$cEmailField;
if($datad = $rs-> fetchAssoc()){	
	if($datad[$cDisplayNameField])
		$value = $datad[$cDisplayNameField];
	else
		$value = $datad[$cEmailField];
}
DB::SetConnection(DB::DefaultConnection());
;
		return $value;
	}
				if($table=="forumtopics" && $field=="topic")
	{
		$value = "<a class='topic-link' href='".GetTableLink("forumreplies","list","mastertable=forumtopics&masterkey1=".$data["id"])."'>".$data["topic"]."</a>";
;
		return $value;
	}
				if($table=="forumtopics" && $field=="topic2")
	{
		if($data["solved"] === "1"){
	$value = "[SOLVED] ".$value;
}
...
$value = getValueHighlighted($value,$words);

$cur_user = Security::currentUserData();
if($_SESSION["User_Type"] == "admin" ){
	$value = $value." <b><a type='button'  class='btn-link   
	 glyphicon glyphicon-pencil' href='".$_SESSION["base_url"]."forumtopics_edit.php?editid1=".$data["id"]."'></a></b>";
};
		return $value;
	}
				if($table=="forumtopics" && $field=="startedby" && $ptype=="view")
	{
		global $cDisplayNameField,$cUserpicField,$cUserNameField;
DB::SetConnection(DB::ConnectionByTable(Security::getLoginTable()));
...
DB::SetConnection(DB::DefaultConnection());
$profileData = getUserData($data["startedby"]);
if( is_null( $profileData["image"] )) {
	$value = "<div class='user-circle'><span class='user-initials'>".substr(strtoupper($record[$cDisplayNameField]),0,1)."</span></div>";
} else {
	$value = "<img height='40px' class='avatar' src='avatars/".$profileData["image"]."'>";
}
if($_SESSION["user_id"])
	$value = "<a href='#' data-username='".$record[$cUserNameField]."' class='userprofile' displayname='".$record[$cDisplayNameField]."'>".$value."</a>";
;
		return $value;
	}
				if($table=="forumtopics" && $field=="startedby" && $ptype=="list")
	{
...
if($_SESSION["user_id"])
	$value = "<a href='#' class='userprofile' data-username='".$record[$cUserNameField]."' displayname='".$record[$cDisplayNameField]."' userid=".$data["startedby"].">".$value."</a>";
;
		return $value;
	}
				if($table=="forumtopics" && $field=="startedby" && $ptype=="print")
	{
...
if ($data["image"]=="") {
	...
	$value = "<img height='40px' class='avatar' src='avatars/".$data["image"]."'>";
}

if($_SESSION["user_id"])
	$value = "<a href='#' class='userprofile' displayname='".$record[$cDisplayNameField]."' userid=".$data["startedby"].">".$value."</a>";
;
		return $value;
	}
				if($table=="forumtopics" && $field=="startedby" && $ptype=="export")
	{
...
;
		return $value;
				...
				if($table=="forumtopics" && $field=="startedby" && $ptype=="masterlist")
	{
...
$record = $rs->fetchAssoc();
DB::SetConnection(DB::DefaultConnection());
global $cDisplayNameField,$cUserpicField;
$profileData = getUserData($data["startedby"]);
if(is_null( $profileData["image"] ) ) {
	$value = "<div class='user-circle'><span class='user-initials'>".substr(strtoupper($record[$cDisplayNameField]),0,1)."</span></div>";
} else {
...
if($_SESSION["user_id"])
	$value = "<a href='#' data-username='".$data[$cUserNameField]."' class='userprofile' displayname='".$record[$cDisplayNameField]."' userid=".$data["startedby"].">".$value."</a>";
;
		return $value;
	}
				if($table=="forumtopics" && $field=="startedby" && $ptype=="masterprint")
	{
...
;
		return $value;
				...
				if($table=="forumtopics" && $field=="startedby2")
	{
		DB::SetConnection(DB::ConnectionByTable(Security::getLoginTable()));
$rs = DB::Select(Security::getLoginTable(),array($_SESSION["loginKeyField"] => $data["startedby"]));
$record = $rs->fetchAssoc();
DB::SetConnection(DB::DefaultConnection());
global $cDisplayNameField;
$value = $record[$cDisplayNameField];;
		return $value;
	}
				if($table=="forumtopics" && $field=="categoryid" && $ptype=="view")
	{
		global $cman;
$conn = $cman->byTable( "forumtopics" );
if($conn->dbType == nDATABASE_Access)
	$url = GetTableLink("forumtopics","list","q=(categoryid~equals~".$data["categoryid"].")");
else
	$url = GetTableLink("forumtopics","list","q=(categoryid~contains~".$value.")");

$value = "<span class=category-info><a href='".$url."'><span class=category-badge style='background:".$data["color"]."' /></span><span class=category-name>".$value."</span></a></span>";;
		return $value;
	}
				if($table=="forumtopics" && $field=="categoryid" && $ptype=="list")
	{
		global $cman;
$conn = $cman->byTable( "forumtopics" );
if($conn->dbType == nDATABASE_Access)
...
$value = "<span class=category-info><a href='".$url."'><span class=category-badge style='background:".$data["color"]."' /></span><span class=category-name>".$value."</span></a></span>";;
		return $value;
				...
				if($table=="forumtopics" && $field=="categoryid" && $ptype=="print")
	{
		global $cman;
$conn = $cman->byTable( "forumtopics" );
if($conn->dbType == nDATABASE_Access)
...
$value = "<span class=category-info><a href='".$url."'><span class=category-badge style='background:".$data["color"]."' /></span><span class=category-name>".$value."</span></a></span>";;
		return $value;
				...
				if($table=="forumtopics" && $field=="categoryid" && $ptype=="export")
	{
		global $cman;
$conn = $cman->byTable( "forumtopics" );
if($conn->dbType == nDATABASE_Access)
...
$value = "<span class=category-info><a href='".$url."'><span class=category-badge style='background:".$data["color"]."' /></span><span class=category-name>".$value."</span></a></span>";;
		return $value;
				...
				if($table=="forumtopics" && $field=="categoryid" && $ptype=="masterlist")
	{
		global $cman;
$conn = $cman->byTable( "forumtopics" );
if($conn->dbType == nDATABASE_Access)
...
$value = "<span class=category-info><a href='".$url."'><span class=category-badge style='background:".$data["color"]."' /></span><span class=category-name>".$value."</span></a></span>";;
		return $value;
				...
				if($table=="forumtopics" && $field=="categoryid" && $ptype=="masterprint")
	{
		global $cman;
$conn = $cman->byTable( "forumtopics" );
if($conn->dbType == nDATABASE_Access)
...
$value = "<span class=category-info><a href='".$url."'><span class=category-badge style='background:".$data["color"]."' /></span><span class=category-name>".$value."</span></a></span>";;
		return $value;
				...
				if($table=="forumtopics" && $field=="activity")
	{
		$d = strtotime($value);
if ($value!="") {
	$hours = (strtotime(runner_date_format("Y-m-d H:i:s")) - $d)/60/60;
	if ($hours<1) {
			$value = round(((strtotime(runner_date_format("Y-m-d H:i:s")) - $d)/60),0)."m";
	}	else if ($hours<24) {
			$value = round($hours,0)."h";
	} else if ($hours>=24 && $hours<720)
			$value = round(($hours/24),0)."d";
	else { 
			$value = runner_date_format("M`y",$d);
	}
			
};
		return $value;
	}
				if($table=="forumtopics" && $field=="question")
	{
		include_once "include/Parsedown.php";
$Parsedown = new Parsedown();
$value = htmlspecialchars_decode($Parsedown->text($value));
$soForumtopics = SearchClause::getSearchObject( "forumtopics" );
$words = $soForumtopics->getSearchToHighlight("topic") ;
$value = getValueHighlighted($value,$words);
...
$value=str_replace("\n\r","<br>",$value);
$value=str_replace("\n","<br>",$value);
$value=str_replace("<pre><code>","<code>",$value);
$value=str_replace("</code></pre>","</code>",$value);;
		return $value;
	}
				if($table=="forumsubscribers" && $field=="userid")
	{
		
DB::SetConnection(DB::ConnectionByTable(Security::getLoginTable()));
$rs = DB::Select(Security::getLoginTable(),array($_SESSION["loginKeyField"] => $data["userid"])); 
$record = $rs->fetchAssoc();
DB::SetConnection(DB::DefaultConnection());
global $cDisplayNameField,$cUserpicField;;
if(is_null( $_SESSION["forumudata"]["image"] )) {
	...
	$value = "<img height='40px' class='avatar' src='avatars/".$_SESSION["forumudata"]["image"]."'>";
};
		return $value;
	}
				if($table=="forumsubscribers" && $field=="topicid")
	{
		$value="<a href='forumsubscribers_list.php?unsub=".$value."' >Unsubscribe</a>";;
		return $value;
	}
				if($table=="forumsubscribers" && $field=="topic")
	{
		$value = "<a href='forumreplies_list.php?mastertable=forumtopics&masterkey1=".$data["topicid"]."'>".$value."</a>";;
		return $value;
	}
				if($table=="forumsubscribers" && $field=="categoryid")
	{
		$value="<a href='forumsubscribers_list.php?unsub=".$data["topicid"]."' >Unsubscribe</a>";
;
		return $value;
	}
				if($table=="forumtopics_view" && $field=="activity")
	{
		$d = strtotime($value);
if ($value!="") {
	$hours = (strtotime(runner_date_format("Y-m-d H:i:s")) - $d)/60/60;
	if ($hours<1) {
		...
		return $value;
				...
				if($table=="forumsubscribers_dash" && $field=="userid")
	{
		DB::SetConnection(DB::ConnectionByTable(Security::getLoginTable()));
								$rs =DB::Select(Security::getLoginTable(),array($_SESSION["loginKeyField"] => $data["userid"])); 
								$record = $rs->fetchAssoc();
...
if ($record["image"]=="") {
	...
	$value = "<img height='40px' class='avatar' src='avatars/".$record["image"]."'>";
};
		return $value;
	}
				if($table=="forumsubscribers_dash" && $field=="topicid")
	{
		$value="<a href='forumsubscribers_list.php?unsub=".$value."' >Unsubscribe</a>";;
		return $value;
	}
				if($table=="forumsubscribers_dash" && $field=="topic")
	{
		;
		return $value;
	}
...
{
	$value = "";
				if($table=="forumusers_data" && $field=="image")
	{
		;
		return $value;
	}
