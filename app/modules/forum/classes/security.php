	...
	*  For AD it is <provider code><username>
	...
	public static function  fillTablesOwnerId( $data, $provider ) {
		...
		if( $securityType == stAD && !$provider["useDbGroups"] ) {
			...
			storageSet( "_forumreplies_OwnerID", $userId );
						storageSet( "_forumtopics_OwnerID", $userId );
						storageSet( "_forumtopics_view_OwnerID", $userId );
		} else {
						storageSet( "OwnerID", $data["id"] );
			storageSet( "_forumreplies_OwnerID", $data["id"] );
						storageSet( "_forumtopics_OwnerID", $data["id"] );
						storageSet( "_forumtopics_view_OwnerID", $data["id"] );
		}
	}
