...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="hd_tickets" && $field=="id")
	{
		$username = Security::getUserName();
$userdata = Security::getUserData($username);
if($userdata["usertype"]=="user")
	$value = DBLookup("select count(".AddFieldWrappers("id").") from ".AddTableWrappers("hd_messages")." where ".AddFieldWrappers("directionid")."<>3 and ".AddFieldWrappers("ticketid")."=".$value);
elseif($userdata["usertype"]=="customer"){
		$value = DBLookup("select count(".AddFieldWrappers("id").") from ".AddTableWrappers("hd_messages")." where ".AddFieldWrappers("ticketid")."=".$value);
}
else
	$value = DBLookup("select count(".AddFieldWrappers("id").") from ".AddTableWrappers("hd_messages")." where ".AddFieldWrappers("ticketid")."=".$value);
;
		return $value;
				...
				if($table=="hd_tickets" && $field=="created")
	{
		$value = date("D m/d/Y h:i a",strtotime($value));;
		return $value;
	}
				if($table=="hd_tickets" && $field=="subject")
	{
		//if(strlen($value)>40)
//	$value = substr($value,0,40)."...";
$value = "<a class=alink href='hd_messages_list.php?masterkey1=".$data["id"]."&mastertable=hd_tickets'>".$value."</a>";
;
		return $value;
	}
				if($table=="hd_tickets" && $field=="statusid" && $ptype=="view")
	{
		$status_color = DBLookup("select ".AddFieldWrappers("statuscolor")." from ".AddTableWrappers("hd_status")." where ".AddTableWrappers("id")."=".$data["statusid"]);
$value = "<span data-clientstatus style='color: ".$status_color.";border: 2px solid ".$status_color.";'>".$value."</span>";;
		return $value;
	}
				if($table=="hd_tickets" && $field=="statusid" && $ptype=="export")
	{
		if($_SESSION["current_user_type"] === "customer"){
if($value === "Answered") $value = "<span class='status_span green_status'>".GetCustomLabel("open")."</span>";
	elseif($value === "Unprocessed") $value = "<span class='status_span green_status'>".GetCustomLabel("awaiting_for_response")."</span>";
		else $value = "<span class='status_span close_status'>".$value."</span>";
}
;
		return $value;
	}
				if($table=="hd_tickets" && $field=="statusid" && $ptype=="masterlist")
	{
		if($_SESSION["current_user_type"] === "customer"){
if($value === "Answered") $value = "<span class='status_span green_status'>Awaiting for response</span>";
	elseif($value === "Unprocessed") $value = "<span class='status_span green_status'>Open</span>";
		else $value = "<span class='status_span close_status'>".$value."</span>";
}
;
		return $value;
	}
				if($table=="hd_tickets" && $field=="statusid" && $ptype=="masterprint")
	{
		if($_SESSION["current_user_type"] === "customer"){
if($value === "Answered") $value = "<span class='status_span green_status'>".GetCustomLabel("awaiting_for_response")."</span>";
	elseif($value === "Unprocessed") $value = "<span class='status_span green_status'>".GetCustomLabel("open")."</span>";
		else $value = "<span class='status_span close_status'>".$value."</span>";
}
;
		return $value;
	}
				if($table=="hd_tickets" && $field=="priorityid")
	{
		if($_SESSION["current_user_type"] === "customer") $value = "";;
		return $value;
	}
				if($table=="hd_tickets" && $field=="categoryid")
	{
		if(!empty($value)) $value = "<b class='labelb'>".GetCustomLabel("category").":</b> ".$value;
	else $value = "";;
		return $value;
	}
				if($table=="hd_tickets" && $field=="ticketemail")
	{
		$email = $value;
$strlen = strlen($value);
if($strlen >= 32){
	$email = substr($email,0,20)."...".substr($email,-10);

}
$value = "<a class='alink' href='hd_messages_list.php?masterkey1=".$data["id"]."&mastertable=hd_tickets'>".$email."</a>";;
		return $value;
	}
				if($table=="hd_tickets" && $field=="assignedid")
	{
		if($value !==""){
	$curUser = Security::currentUserData();
	$link = GetTableLink("hd_tickets","list")."?qs=".$value."&f=(assignedid~equals~".$data["assignedid"].")";
	if($curUser["usertype"] === "customer"){
		$value = "";
	}
	else{
			$value = "<a title='".str_replace("%user%" , $curUser[GetDisplayNameField()], GetCustomLabel("tickets_assigned"))."' ".($data["assignedid"]==$curUser[$_SESSION["user_keyfield_for_query"]]?"class='curuser'":"")." href='".$link."'>".$value."</a>";
	}
}
;
		return $value;
	}
				if($table=="hd_tickets" && $field=="attached")
	{
		$cnt = DBLookup("select count(".AddFieldWrappers("id").") from ".AddTableWrappers("hd_messages")." where ".AddFieldWrappers("attachment")." not like '' and ".AddFieldWrappers("ticketid")."=".$value); 
if(intval($cnt) > 0)
$value = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-paperclip" viewBox="0 0 16 16">
...
if($userdata["usertype"]=="user")
	...
	$value = DBLookup("select count(".AddFieldWrappers("id").") from ".AddTableWrappers("hd_messages")." where ".AddFieldWrappers("ticketid")."=".$value);;
		return $value;
				...
				if($table=="hd_tickets" && $field=="clientinfo")
	{
		if($_SESSION["current_user_type"] !== "customer") {
	$client_rs = DB::Select("hd_users",array("id" => $data["clientid"]));
	$client = $client_rs->fetchAssoc();
	$value = "<span class='client_info'><b>".GetCustomLabel("Client").": </b><a title='".GetCustomLabel("show_customer_tickets")."' href='".GetTableLink("hd_tickets","list")."?qs=".$client[GetUserNameField()]."'>".$client[GetDisplayNameField()]."</a>, ".$client[GetUserNameField()]."</span>";
}
else{
	$value = "";
};
		return $value;
				...
				if($table=="hd_tickets" && $field=="updated" && $ptype=="view")
	{
		$value = date("D m/d/Y h:i a",strtotime($value));;
		return $value;
	}
				if($table=="hd_tickets" && $field=="updated" && $ptype=="list")
	{
		$value = getHowAgo($value);;
		return $value;
	}
				if($table=="hd_tickets" && $field=="updated" && $ptype=="export")
	{
		$value = date("D m/d/Y h:i a",strtotime($value));;
		return $value;
	}
				if($table=="hd_tickets" && $field=="updated" && $ptype=="masterlist")
	{
		$value = date("D m/d/Y h:i a",strtotime($value));;
		return $value;
	}
				if($table=="hd_tickets" && $field=="updated" && $ptype=="masterprint")
	{
		$value = date("D m/d/Y h:i a",strtotime($value));;
		return $value;
	}
				if($table=="hd_tickets" && $field=="client_status" && $ptype=="view")
	{
		$value = "<span data-clientstatus='".$data["client_status"]."'>".$value."</span>";

;
		return $value;
	}
				if($table=="hd_messages" && $field=="created")
	{
		if($value)
	$value = date("l, F d, Y h:i a",strtotime($value));;
		return $value;
	}
				if($table=="hd_messages" && $field=="created_ago")
	{
		$value = getHowAgo($value);
if( $data["directionid"]==1 ){
	$value="<span class='user_caption'><i class='fa fa-user'></i><p class='name_and_time'>You ".$value."</p></span>";
}
if( $data["directionid"]==2 ){
	$value="<span class='user_caption'><i class='fa fa-comment'></i><p class='name_and_time'>".$data["from_name"]." ".$value."</p></span>";
}

;
		return $value;
	}
				if($table=="hd_messages" && $field=="to_email")
	{
		if($value)
	$value = "&lt;".$value."&gt;";
if(!empty($data["cc"])) $value.="<span class='additional_emails'><b>CC:</b> ".$data["cc"]."</span>";
if(!empty($data["bcc"])) $value.="<span class='additional_emails'><b>BCC:</b> ".$data["bcc"]."</span>";;
		return $value;
	}
				if($table=="hd_messages" && $field=="from_email")
	{
		$value = "&lt;".$value."&gt;";;
		return $value;
	}
				if($table=="hd_messages" && $field=="reply")
	{
		if($data["directionid"]!=3){
	$username = Security::getUserName();
	$userdata = Security::getUserData($username);
	if($data["directionid"] == 1)
		$value = "<atype='button' class='btn btn-default btn-sm' href='javascript:void(0);' title='' replyid=".$value." id='reply_".$value."'>".GetCustomLabel("reply")."</a>";
	if($data["directionid"] == 2 && $userdata["usertype"]!="user")
		$value = "<atype='button' class='btn btn-default btn-sm' href='javascript:void(0);' title='' resendid=".$value." id='resend_".$value."'>".GetCustomLabel("resend")."</a>";
}
else
	$value = "";;
		return $value;
	}
				if($table=="hd_messages" && $field=="deleterec")
	{
		$value = "<atype='button' class='btn btn-default delrecord btn-sm' href='javascript:void(0);' title='' recid=".$value.">Delete</a>";;
		return $value;
	}
				if($table=="hd_folders" && $field=="id")
	{
		$username = Security::getUserName();
$userdata = Security::getUserData($username);
$value = DBLookup("select count(".AddFieldWrappers("id").") from ".AddTableWrappers("hd_tickets")." where ".AddFieldWrappers("folderid")."=".$value);;
		return $value;
	}
				if($table=="hd_folders" && $field=="foldername")
	{
		$value = "<a href='hd_tickets_list.php?f=(folderid~equals~".$data["id"].")'>".$value."</a>";
;
		return $value;
	}
				if($table=="hd_folders" && $field=="lastticket")
	{
		if($value)
	$value = date("l, F d, Y h:i a",strtotime($value));;
		return $value;
	...
	{
		$value = "<a target='_blank' href='main_export.php?a=export&amp;selection[]=".$data["ArticleID"]."'>".GetCustomLabel("export_article")."</a>";;
		return $value;
	}
				if($table=="kbarticles" && $field=="ArticleID2")
	{
		$value = "<a target='_blank' href='main_print.php?selection[]=".$data["ArticleID"]."'>".GetCustomLabel("printer_friendly_version")."</a>";;
		return $value;
	}
				if($table=="kbarticles" && $field=="ArticleIDedit")
	{
		$value = "<a href='kbarticles_edit.php?editid1=".$value."'>".GetCustomLabel("edit")."</a>";
;
		return $value;
	}
	...
	{
		$value = "<img src='images/kbbullet.gif'>&nbsp;<a href='kbarticles_view.php?editid1=".$data["ArticleID"]."'>".$value."</a>";;
		return $value;
	}
				if($table=="kbarticles" && $field=="Title" && $ptype=="print")
	{
		$value = "<img src='images/kbbullet.gif'>&nbsp;<a href='kbarticles_view.php?editid1=".$data["ArticleID"]."'>".$value."</a>";;
		return $value;
	}
				...
				if($table=="kbarticles" && $field=="Title" && $ptype=="masterprint")
	{
		...
		return $value;
				...
				if($table=="kbarticles" && $field=="Views")
	{
		$value = "(View: ".$value.")";;
		return $value;
	}
	...
	{
		$value = "<a target='_blank' href='main_print.php?selection[]=".$data["ArticleID"]."'>".GetCustomLabel("export_article")."</a>";;
		return $value;
	}
...
;
	...
	{
		$value="<a href='main_list.php?page=list1&q=(Category~equals~".$value.")'>".$value."</a>";;
		return $value;
	}
				if($table=="main" && $field=="Category" && $ptype=="list")
	{
		if($_SESSION["UserID"]=="admin")
	$value="<a href='kbarticles_list.php?q=(Category~equals~".$value.")'>".$value."</a>";
else
	$value="<a href='main_list.php?page=list1&q=(Category~equals~".$value.")'>".$value."</a>";;
		return $value;
	}
	...
	{
		$value = "<a href='main_view.php?editid1=".$data["ArticleID"]."'>".$value."</a>";;
		return $value;
	}
...
{
	$value = "";
				if($table=="hd_messages" && $field=="attachment")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="hd_messages" && $field=="from_email")
	{
		return $_SESSION["current_user_email"];
	}
	return "";
}
