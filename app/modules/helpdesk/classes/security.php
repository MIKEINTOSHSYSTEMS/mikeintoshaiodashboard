	...
	*  For AD it is <provider code><username>
	...
	public static function  fillTablesOwnerId( $data, $provider ) {
		...
		if( $securityType == stAD && !$provider["useDbGroups"] ) {
			...
			storageSet( "_hd_tickets_OwnerID", $userId );
						storageSet( "_hd_mailtemplates_OwnerID", $userId );
		} else {
						storageSet( "OwnerID", $data["id"] );
			storageSet( "_hd_tickets_OwnerID", $data["id"] );
						storageSet( "_hd_mailtemplates_OwnerID", $data["id"] );
		}
	}
