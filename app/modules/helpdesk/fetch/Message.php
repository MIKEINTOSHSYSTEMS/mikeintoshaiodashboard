...
class Message
{
     ...
     *
	 * @param 	array	$idsToSkip - array with list of message IDs that should be skipped to save the bandwidth
	 * @return 	boolean	true if message was loaded, false if message needs to be skipped
     */
    protected function loadMessage($idsToSkip=null)
    {
		...
		// check if we retrieved this message previously
		if ($idsToSkip) {
			if (array_key_exists($messageOverview->message_id, $idsToSkip)) {
				echo "Skipping body ".htmlspecialchars($messageOverview->message_id)."<br>";	
				return false;
			}
		}
		if(isset($messageOverview->message_id))
			echo "Retrieving body ".htmlspecialchars($messageOverview->message_id)."<br>";

		$this->subject = $this->memodecode(isset($messageOverview->subject) ? $messageOverview->subject : "" , self::$charset);
	...
	public static function memodecode($text, $targetCharset = 'utf-8')
    {
        if (null === $text) {
            return null;
        }
