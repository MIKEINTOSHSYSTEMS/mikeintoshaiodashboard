...
//			$mailboxes = imap_list($imapStream, $this->getServerString(), '*');
//			print_r($mailboxes);
    ...
    public function search($criteria = 'ALL', $limit = null)
    {
		...
		$folders = array("INBOX");
		$messages = array();
		if($_SESSION["emailfolder"] != "fetch INBOX emails only"){
			$folderslist = imap_listmailbox($imapStream, $this->getServerString(), "*");
			foreach($folderslist as $f){
				$p = strpos($f,"}");
				$folders[] = substr($f,$p+1);
			}
		}
		foreach($folders as $folder){
			$imapStream = imap_open($this->getServerString().$folder, $this->username, $this->password, $this->options, 1);
            $this->imapStream = $imapStream;
			if ($results = imap_search($imapStream, $criteria, SE_UID)) {
     ...
     *
     * @param  	null|int  	$limit
	 * @param 	array		$idsToSkip - array with list of message IDs that should be skipped to save the bandwidth
     * @return 	Message[]
     */
    public function getMessages($limit = null, $idsToSkip = null)
    {
		
		if($_SESSION["emaildays"] != 0){
			$date = date("m/d/Y", strtotime(date("Y-m-d"))-$_SESSION["emaildays"]*24*60*60);
