...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="doc_files" && $field=="description")
	{
		$value = GetCustomLabel("created")." ".runner_date_format('d M, Y',strtotime($data["created"]));

if ($data["file_type"]=="file") {
	$fileArray = my_json_decode($data["file"]);
	$value.="&nbsp;&nbsp;&nbsp;".formatBytes($fileArray[0]["size"],1);
	$downloads = 0;
	if($data["downloads"])
		$downloads = $data["downloads"];
	$value.="&nbsp;&nbsp;&nbsp;".GetCustomLabel("downloads").":&nbsp;<span id='download'>".$downloads."</span>";
}
else {
	$arr=getFolderInfo($data["id"]);

	// display folder info
	if ($arr["file"]>0 || $arr["folder"]>0) {
		$str=" (";
		if ($arr["file"]>0) {
			$str.=$arr["file"]." ".GetCustomLabel("files");
		}
		if ($arr["file"]>0 && $arr["folder"]>0)
			$str.=", ";
		if ($arr["folder"]>0) {
			$str.=$arr["folder"]." ".GetCustomLabel("folders");
		}
		$str.=")";
...
if(!$isCatStr && $isVersions){
	$rse = DB::Query("select * from doc_files_versions where docid=".$data["id"]);
	if($datae = $rse->fetchAssoc())
		$value.="<br><a href='#' id='fileversion_".$data["id"]."' style='font-size:11;position: relative;top: 5px;'>".GetCustomLabel("all_versions")."</a>";
}
;
		return $value;
	}
				if($table=="doc_files" && $field=="hash")
	{
		global $strTableName;
$perm = GetUserPermissions($strTableName);

if (Security::getUserName()=="Guest" || $data["ownerid"]!=$_SESSION["user_id"] && $_SESSION["user_type"]!="admin"){
	$value="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
}
else{
		$val = $value;
		...
		$value = "<a class='btn btn-link docman-share filedhash' href=# onclick='Runner.docManager.share(\"".$val."\",".$data["id"]."); return false;'>".GetCustomLabel("share")."</a>";
		if($data["file_type"]=="file")
			$value.="<span class='btn btn-link filedhash' onclick='Runner.docManager.sendfile(\"".$val."\",\"".$name."\"); return false;'>".GetCustomLabel("send")."</span>";
}
...
;
		return $value;
				...
				if($table=="doc_files" && $field=="parent_folder_id")
	{
		if((postvalue("qs") || postvalue("q")))
	$value = getSearchPath($data["parent_folder_id"]);
else
	$value="";
;
		return $value;
	}
				if($table=="doc_files" && $field=="file_type")
	{
		global $strTableName;
$share_img = "";
$lockcss = "";
if(Security::getUserName()=="Guest" || $_SESSION["user_type"]!="admin" && $data["ownerid"]!=$_SESSION["user_id"])
{
	...
	if($data["share_ro"] == "Read only") //strpos($perm, "E")==false
		$lockcss = "-webkit-filter: grayscale(100%);";
}

if ($value=='folder') {
	if($data["share_type"]>0 && $data["share_type"]<4)
		$value="<a href='doc_files_list.php?hash=".$data["hash"]."'><div style='width:100px;'><img src='".GetWebRootPath()."folder.png' style='".$lockcss."'><img src='".GetWebRootPath()."share.png' style='position:relative;left:-35px;top:17px;".$lockcss."'></div></a>";
	else
		$value="<a href='doc_files_list.php?hash=".$data["hash"]."'><div style='width:100px;'><img src='".GetWebRootPath()."folder.png' style='".$lockcss."'></div></a>";
}
if ($data["file_type"]=='file') {
	$fileArray = my_json_decode($data["file"]);
	$name=$fileArray[0]["usrName"];  
	$fileType = getContentTypeByExtension(substr($name, strrpos($name, '.')));
	$isCatStr = DBLookup("select ".addFieldWrappers("isStructure")." from ".AddTableWrappers("doc_settings"));
	if($data["thumb"]){
		if($isCatStr)
			$sess_path = $_SESSION["mkdirForFiles"];
		else
			$sess_path = $_SESSION["show_upload_path"];

		if(isAbsolutePath($sess_path)){
			$image = $sess_path."/".$data["thumb"];
			$imageData = base64_encode(file_get_contents($image));
			$icons = 'data: ' . SupposeImageType($image) . ';base64,' . $imageData;
			$size = runner_getimagesize($sess_path."/".$data["thumb"],$sess_path);
		}
		else{
			$icons = GetWebRootPath().$sess_path."/".$data["thumb"];
			$size = runner_getimagesize(getabspath($sess_path."/".$data["thumb"]),getabspath($sess_path));
		}

		
		$sharecss = "left:-".($size[0]+$size[0]/2-25)."px;top:".($size[1]/2)."px;";
	}
	else{
		$icons = GetWebRootPath()."images/icons/".getIconByFileType($fileType, $name);
		$sharecss = "left:-35px;top:16px;";
	}

	if($data["share_type"]>0 && $data["share_type"]<4)
		$share = "<div style='width:100px;'><img src='".$icons."' style='".$lockcss."'><img src='".GetWebRootPath()."share.png' style='position:relative;".$sharecss.$lockcss."'></div>";
	else
		$share = "<div style='width:100px;'><img src='".$icons."' style='".$lockcss."'></div>";
	if(strtoupper(substr($name,-3)) == "PDF")
		$value = "<a onclick='plus_download($(this));' target=_blank href='pdf.js/web/viewer.html?file=".rawurlencode($name)."&table=".rawurlencode($strTableName)."&field=file&pageType=list&key1=".$data["id"]."&hash=".$data["hash"]."'>".$share."</a>";
	else
		$value = "<a onclick='plus_download($(this));' href='doc_files_list.php?file=".rawurlencode($name)."&table=".rawurlencode($strTableName)."&field=file&pageType=list&key1=".$data["id"]."&hash=".$data["hash"]."'>".$share."</a>";
};;
		return $value;
	}
				if($table=="doc_files" && $field=="name")
	{
		global $strTableName;
if ($data["file_type"]=='file') {
	$fileArray = my_json_decode($data["file"]);
	...
	if(strtoupper(substr($name,-3)) == "PDF")
		$value = "<a target=_blank href='pdf.js/web/viewer.html?file=".rawurlencode($name)."&table=".rawurlencode($strTableName)."&field=file&pageType=list&key1=".$data["id"]."&hash=".$data["hash"]."' onclick='plus_download($(this));'>".$showname."</a>";
	else
		$value = "<a href='doc_files_list.php?file=".rawurlencode($name)."&table=".rawurlencode($strTableName)."&field=file&pageType=list&key1=".$data["id"]."&hash=".$data["hash"]."' onclick='plus_download($(this));'>".$showname."</a>";
}
else if ($data["file_type"]=='folder') {
	$name = $data["name"];
	if(strlen($name)>35)
		$name = substr($name,0,32)."...";
	$value="<a href='doc_files_list.php?hash=".$data["hash"]."'>".$name."</a>";
}
;
		return $value;
	}
				if($table=="Share" && $field=="hash")
	{
		
		$url = projectURL() . GetTableLink( "doc_files", "list" );	
		$_SESSION["share_link"] = $url;
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="doc_users" && $field=="usertype")
	{
		return "user";
	}
				if($table=="Share" && $field=="share_date")
	{
		return now();
	}
				if($table=="Share" && $field=="share_ro")
	{
		return "Read / Write";
	}
	return "";
