...
$custom_labels["English"] = array();
	$custom_labels["English"]['login_header'] = "Use <b>admin@test.com/admin</b> to login";
	$custom_labels["English"]['current_version'] = "Current version";
	$custom_labels["English"]['version'] = "Version";
	$custom_labels["English"]['file_sent'] = "File was sent to %user%";
	$custom_labels["English"]['were_deleted'] = "%count% file(s) were deleted";
	$custom_labels["English"]['remove'] = "Remove";
	$custom_labels["English"]['drop_file'] = "Drop files here to upload";
	$custom_labels["English"]['download'] = "Download";
	$custom_labels["English"]['delete'] = "Delete";
	$custom_labels["English"]['want_to_delete'] = "Do you want to delete this file?";
	$custom_labels["English"]['dest_file_exist'] = "The destination already has a file named";
	$custom_labels["English"]['dest_replace_file'] = "Replace the file in the destination";
	$custom_labels["English"]['skip_file'] = "Skip this file";
	$custom_labels["English"]['save_as'] = "Save as";
	$custom_labels["English"]['close'] = "Close";
	$custom_labels["English"]['files_added'] = "File(s) not added";
	$custom_labels["English"]['change_settings'] = "If you change this setting you won't be able to see existing files. Continue?";
	$custom_labels["English"]['user_shared_file'] = "%username% shared a %file_type% with you";
	$custom_labels["English"]['noty_sent'] = "Notification sent to \$email";
	$custom_labels["English"]['read_write'] = "Read / Write";
	$custom_labels["English"]['upload_path_exist'] = "Upload folder %upload_path% not exist";
	$custom_labels["English"]['select_all'] = "Select all";
	$custom_labels["English"]['downloads'] = "Downloads";
	$custom_labels["English"]['all_versions'] = "All versions";
	$custom_labels["English"]['created'] = "Created";
	$custom_labels["English"]['files'] = "file(s)";
	$custom_labels["English"]['folders'] = "folder(s)";
	$custom_labels["English"]['share'] = "Share";
	$custom_labels["English"]['send'] = "Send";
	$custom_labels["English"]['create_folder'] = "Create folder";
	$custom_labels["English"]['save'] = "Save";
	$custom_labels["English"]['to'] = "To";
	$custom_labels["English"]['subj'] = "Subject";
	$custom_labels["English"]['body'] = "Body";
	$custom_labels["English"]['send_file'] = "Send file";
	$custom_labels["English"]['recipient'] = "Recipient not selected";
	$custom_labels["English"]['cancel'] = "Cancel";
...
$globalSettings["LandingPageId"] = "menu_DocManager";

$globalSettings["ProjectLogo"] = array();
$globalSettings["ProjectLogo"]["English"] = "DocManager";
...
$styleOverrides["doc_files_list"] = array(
	"theme" => "cerulean",
	"size" => "normal",
	"path" => "styles\\bootstrap\\custom\\doc_files_list",
);
...
$globalSettings["usersDatasourceTable"] = "doc_users";


$globalSettings["jwtSecret"] = "OqNEdWbuOxqpkdvw1bGu";
...
$projectBuildKey = "235_1668160259";
...
$tableCaptions["English"]["doc_files"] = "Doc Files";
$tableCaptions["English"]["doc_settings"] = "Doc Settings";
$tableCaptions["English"]["doc_users"] = "Doc Users";
$tableCaptions["English"]["share_type"] = "Share Type";
$tableCaptions["English"]["Add_folder"] = "Add Folder";
$tableCaptions["English"]["Share"] = "Share";
$tableCaptions["English"]["doc_savemethod"] = "Doc Savemethod";
$tableCaptions["English"]["doc_files_versions"] = "Doc Files Versions";
...
include("docman_functions.php");

if( !$_SESSION["path_folder"] || postvalue("folder") === "0" )
	$_SESSION["path_folder"] = array();
	
if( !$_SESSION["path_file"] )
	$_SESSION["path_file"] = array();

//if ( (Security::getUserName() == "Guest" || strlen(Security::getUserName())==0) && !postvalue("hash") && getFileNameFromURL()=="doc_files_list.php") {
...
// regular user
// root folder
if (Security::getUserName() != "Guest" && strlen(postvalue("folder")) && getFileNameFromURL()=="doc_files_list.php") {
	$_SESSION["current_folder"] = postvalue("folder");
}
global $conn;
$conn=db_connect();

if(postvalue("folder")==0 && getFileNameFromURL()=="doc_files_list.php")
{
	$_SESSION["hash"] = "";
	$_SESSION["single_file"]=false;
}

if($_SESSION["upload_path"] && !is_dir($_SESSION["upload_path"]))
{
	mkdir($_SESSION["upload_path"], 0777,  true);
}

if(postvalue("a")!="delete")
	$_SESSION["hash_for_delete"] = 0;
...
if (postvalue("hash") && getFileNameFromURL()=="doc_files_list.php"){
	...
	// store hash of shared folder in session variable
	// we are going to need it while building breadcrumbs menu for shared link visitors
	if ((strlen(Security::getUserName())==0 || Security::getUserName()=="Guest")) {
			setcookie("username","",time()-365*1440*60);
			setcookie("password","",time()-365*1440*60);
			// autologin shared link user
			//#tablename doc_files#
			$_SESSION["_doc_files_OwnerID"]=$data["ownerid"];
	...
	$_SESSION["single_file"]=$data["file_type"]=="file" ? true : false;
	if($data["file_type"]=="folder")
  {
			$_SESSION["current_folder"]=$data["id"];
			//if($data["ownerid"] == Security::getUserName())
					setPathFolder($data["id"]);

			if(!$_SESSION["guest_startFolder"])
				$_SESSION["guest_startFolder"] = $_SESSION["current_folder"];
  }
	else
  {
			$_SESSION["current_folder"]=$data["parent_folder_id"];
		  //setPathFile($data["id"]);
  }
}
$_SESSION["path"]=getPathArray($_SESSION["current_folder"]);
