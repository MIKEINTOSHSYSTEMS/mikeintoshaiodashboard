...
/*
	...
	*/
	...
	/**
	*/
	function getFilterCondition( $controls, $ignoreFilterField = "" )
	{
...
/*		
		...
		{	
			$fName = $this->searchUnEscape( $filter[0] );
			if( $fName == $ignoreFilterField ) {
				continue;
			}
			if( !$this->filteredFields[$fName] ) {
				$this->filteredFields[$fName] = array( "values"=> array(), "parentValues" => array() );
			}
			$filterType = $filter[1];
			...
			if( $filter[4] )
			{
				$parentValuesString = $this->searchUnEscape( $filter[4] ); // values glued by '|'
				$parentValues = $this->getUnescapedFValues( $parentValuesString );
				$this->filteredFields[$fName]["parentValues"] = $parentValues;
			}
			if( !$fieldConditions[ $fName ] ) {
				$fieldConditions[ $fName ] = array();
			}
			$fieldConditions[ $fName ][] = $this->getFilterConditionByType( $filterType, $fName, $fValue , $sValue, $parentValues );
			$this->filteredFields[$fName]["values"][] = $fValue;
			// 			$this->filteredFields[$fName] = array("values" => $fData["value"], "where" => $fieldWhere, "parentValues" => $fData["parentValues"] );
		}

		// combine conditions. Conditions for the same field are combined with OR, the results are with AND
		$conditions = array();
		foreach( $fieldConditions as $fc ) {
	...
	*/
	...
	public function getSearchDataCondition( $editControls = null )
	{
		...
		foreach ($searchFields as $ind => $srchF)
		{
			...
			$fieldConditions[ $field ][] = $control->getSearchCondition( $srchF['value1'], $srchF['opt'], $srchF['value2'], $srchF['not'] );
		}

		foreach( $this->customFieldSQLConditions as $field => $sql )
		{
			$fieldConditions[ $field ] = array( DataCondition::SQLCondition( $sql ) );
		}
	...
	protected function getSimpleSearchCondition( $editControls )
	{
		...
		foreach( $this->searchFieldsArr as $field ) {
			...
			foreach($searchForValues as $ind => $searchValue ) {
				...
				$valueConditions[$ind][] = $control->getSearchCondition( $searchFor, $searchOption );
			}
