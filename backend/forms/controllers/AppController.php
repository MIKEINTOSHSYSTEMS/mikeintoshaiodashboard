...
class AppController extends Controller
{
                    ...
                    [
                        ...
                        'matchCallback' => function ($rule, $action) {
                            if (in_array($action->id, ['check', 'preview', 'embed', 'form', 'forms', 'f'])) {
                                ...
                                if ($action->id === 'forms') {
                                    // By Slug
                                    $this->formModel = Form::findOne(['slug' => Yii::$app->request->getQueryParam('slug')]);
                                } else {
                                    // By ID or HashID
                                ...
                                if ($this->formModel) {
                                    ...
                                    if (!$this->formModel->is_private) {
                                        return true;
                                    }

                                    // User has access when he can update submissions
                                    if ($user->can("updateFormSubmissions", ['model' => $this->formModel])) {
                                        return true;
                                    }
                                    ...
                                    if ($user && !$user->isGuest) {
                                        ...
                                        $hasAccess = $this->formModel->shared === Form::SHARED_EVERYONE
                                            || ($this->formModel->shared === Form::SHARED_WITH_USERS && in_array($this->formModel->id, $sharedIds));
     ...
     * @return Form|array|\yii\db\ActiveRecord
     * @throws NotFoundHttpException
     */
