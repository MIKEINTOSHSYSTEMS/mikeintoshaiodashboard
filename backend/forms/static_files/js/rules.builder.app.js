    ...
    "actions": [
        ...
        {label: options.i18n.copy, name: "copy", fields: [
            {"label": options.i18n.from, "name": "original", "fieldType": "select", "options": [
                ...
                {"label": options.i18n.value, "name": "value", "fields": [
                    {"label": "", "name": "originalValue", "fieldType": "textarea"}
                ]}
            ]},
...
App.RulesView = Backbone.View.extend({
    ...
    render: function () {
        ...
        this.collection.each(function (model) {
            that.subViews[model.cid] = new App.RuleView({ model: model });
            fragment.appendChild(that.subViews[model.cid].render().el);
        });
        this.$el.empty();
        this.$el.append(fragment);
        // var t1 = performance.now();
        // console.log("Call to Rules:render took", (t1 - t0), "milliseconds. With", this.collection.length, "components.");
        return this;
    }
});
